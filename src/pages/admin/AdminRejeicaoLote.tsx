...
const AdminRejeicaoLote = () => {
  ...
  const rejectSelected = async () => {
    ...
    if (!rejectReason) { toast({ title: "Erro", description: "Selecione um motivo de rejeição", variant: "destructive" }); return; }
    try {
      await api.batchReject(selectedIds);
  ...
  return (
    <AppLayout pageName="Rejeição em Lote" navItems={ADMIN_NAV} notificationCount={0}>
      {/* Red Banner */}
      <div className="rounded-xl p-7 mb-6 text-primary-foreground" style={{ background: "linear-gradient(135deg, #e53935 0%, #d32f2f 100%)" }}>
        <h2 className="text-[22px] font-semibold mb-1.5">Rejeição de Projetos em Lote</h2>
        <p className="text-sm opacity-90">Rejeite projetos que não atendem aos critérios</p>
      </div>
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border overflow-hidden mb-6">
        <div className="p-5 pb-0 flex justify-between items-center">
          <h3 className="text-base font-semibold">Projetos Pendentes de Rejeição</h3>
        ...
        <div className="p-5 space-y-3">
          {loading ? (
            ...
            <div className="text-center py-10 text-muted-foreground">
              ...
              <h4 className="font-semibold">Nenhum projeto para rejeitar</h4>
      ...
      <div className="bg-card rounded-xl shadow-sm border border-border p-5 mb-6">
        <h3 className="text-base font-semibold mb-4">Motivo da Rejeição</h3>
        <div className="mb-4">
          <label className="block text-sm font-semibold mb-2">Selecione o motivo:</label>
          <select value={rejectReason} onChange={(e) => setRejectReason(e.target.value)} className="w-full px-3 py-2.5 border border-border rounded-lg text-sm bg-card">
            <option value="">-- Selecione --</option>
            <option value="dados-incompletos">Dados Incompletos</option>
            <option value="nao-atende-criterios">Não Atende aos Critérios</option>
            <option value="qualidade-insuficiente">Qualidade Insuficiente</option>
            <option value="fora-escopo">Fora do Escopo</option>
            <option value="outro">Outro</option>
          </select>
        </div>
        <div>
          <label className="block text-sm font-semibold mb-2">Comentários Adicionais:</label>
          <textarea value={rejectComments} onChange={(e) => setRejectComments(e.target.value)} rows={4} placeholder="Deixe um comentário para o pesquisador..." className="w-full px-3 py-2.5 border border-border rounded-lg text-sm bg-card resize-y" />
        </div>
      </div>

      {/* Action Buttons */}
      <div className="flex gap-3">
        <button onClick={rejectSelected} disabled={selectedIds.length === 0} className="px-6 py-3 rounded-lg text-sm font-semibold text-primary-foreground disabled:opacity-50" style={{ background: "#e53935" }}>
...
export default AdminRejeicaoLote;
